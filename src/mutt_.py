...
def evaluate_mutt(all_metrics,corrupt=[],demo=False,num=5,quaed=False):
  """
  ...
  for corruption in (list(corruptions.keys()) + list(g_c.corruptions.keys())):
    if ((corruption in corrupt) or corrupt==[]):
      metrics.coco(*writer.files_json(corruption), corruption=corruption, f=coco_file,metrics=all_metrics,showoff=demo,num=num,quaed=quaed)
